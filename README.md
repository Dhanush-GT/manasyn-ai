...
A production-grade, privacy-first conversational reflection companion built with **Google Gemini**, **Firebase Authentication**, **Cloud Firestore**, and **Google Cloud Run**.

---

## Architecture & Threat Countermeasures

Manasyn was engineered using Google AI Studio configured with strict threat-modeling directives and secure coding standards:

| Threat Zone | Identified Vector | OWASP Standard | Implemented Countermeasure |
| :--- | :--- | :--- | :--- |
| **Input Surfaces** | Malicious injection in reflection prompts; SSRF via custom webhooks | OWASP A03 / LLM02 | Strict schema validation; server-side URL filtering blocking loopback (`127.0.0.1`), private IP ranges (`10.0.0.0/8`), and cloud metadata (`169.254.169.254`). |
...
| **Memory & State** | Cross-user data leakage and unauthorized reading of private reflections | OWASP A01 / Firebase Security | User Data Isolation via owner-bound Firestore security rules (`request.auth.uid == userId`) across all user subcollections. Zero admin read backdoors into reflection text. |
| **Inter-System Comm.** | API key leakage or external endpoint exposure | OWASP A02 / A05 | `GEMINI_API_KEY` managed via Google Cloud Secret Manager; Express proxy routes handle all downstream calls server-side. |

---

## Core Capabilities & Original Features

1. **Conversational Clarity:** Multi-turn conversational journaling that untangles thoughts, helps weigh trade-offs, and structures actionable insights.
...
3. **Places:** Contextual place tagging with adjustable precision (approximate area vs. specific location) without continuous GPS tracking.
...
6. **In-App Feedback:** Direct, owner-isolated Firestore feedback channel with privacy opt-outs and diagnostic inspection.
...
### Deployment & Verification Labeling
